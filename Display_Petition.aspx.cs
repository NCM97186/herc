...
public partial class Auth_AdminPanel_Petition_Management_Display_Petition : System.Web.UI.Page
{
    ...
    {
        Response.Redirect("Petition_Add_Edit.aspx?ModuleID=" + Convert.ToInt32(Request.QueryString["ModuleID"]));
    }
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        ...
        p_Var.rp_id = Convert.ToInt16(p_Var.commandArgs[0]);
    ...
    {
        p_Var.petition_id = Convert.ToInt16(ViewState["id"]);
                ...
                if (p_Var.dSet.Tables[0].Rows.Count > 0)
                {
                    if (Convert.ToInt16(ddlStatus.SelectedValue) == Convert.ToInt16(Module_ID_Enum.Module_Permission_ID.Approved))
                    {
                        ...
                        foreach (GridViewRow row in grdPetition.Rows)
                        {
                            ...
                            petObject.PetitionId = Convert.ToInt16(lblPetitionID.Text);
                            ...
                            for (p_Var.i = 0; p_Var.i < p_Var.dSetCompare.Tables[0].Rows.Count; p_Var.i++)
                            {
                                ...
                                {
                                    if (Convert.ToInt16(lblPetitionID.Text) == Convert.ToInt16(p_Var.dSetCompare.Tables[0].Rows[p_Var.i]["Old_Petition_Id"]))
                                    {
    ...
    public void ChkApprove_Disapprove()
    {
        ...
        foreach (GridViewRow row in grdPetition.Rows)
        {
            ...
            if ((selCheck.Checked == true))
            {
                ...
                ////dsMail = petPetitionBL.getEmailIdForSendingMail(petObject);
                ...
                //        miscellBL.SendEmail(drRow["Petitioner_Email"].ToString().Trim(), "", "", "Multiple Email Testing from Petition during approve", "birendra.kumar@netcreativemind.co.in", "Testing Email");
    ...
    protected void grdPetition_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else if (e.CommandName == "ChangeStatus")
        {
            ...
            petObject.TempPetitionId = Convert.ToInt16(e.CommandArgument.ToString());
        ...
        else if (e.CommandName == "Restore")
        {
            ...
            petObject.PetitionId = Convert.ToInt16(e.CommandArgument);
    ...
    //{
    //    Response.Redirect("PetitionReview_Insert_Update.aspx?ModuleID=" + Convert.ToInt32(Request.QueryString["ModuleID"]));
    ...
    protected void grdOrderPdf_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            petObject.PetitionId = Convert.ToInt16(grdOrderPdf.DataKeys[e.Row.RowIndex].Value.ToString());
            ...
            if (Page.IsValid)
            {
                if (Convert.ToInt16(ddlStatus.SelectedValue) == Convert.ToInt16(Module_ID_Enum.Module_Permission_ID.draft))
                {
                    ...
                    foreach (GridViewRow row in grdPetition.Rows)
                    {
                        ...
                        if ((selCheck.Checked == true))
                        {
                            sbuilder.Append(lblProNumber.Text + "; ");
                            Label lblid = (Label)row.FindControl("lblPetition_ID");
                            p_Var.dataKeyID = Convert.ToInt32(lblid.Text);
                            petObject.TempPetitionId = p_Var.dataKeyID;
                            petObject.StatusId = (int)Module_ID_Enum.Module_Permission_ID.review;
                    ...
                    if (p_Var.Result > 0)
                    {
                        ...
                        {
                            p_Var.sbuilder.Append("You have records for review : " + sbuilder.ToString().Remove(sbuilder.Length-1));
                ...
                else
                {
                    ...
                    if (p_Var.Result > 0)
                    {
                        ...
                        if (p_Var.sbuildertmp.ToString().EndsWith(";"))
                        {
                            p_Var.sbuilder.Append("You have records for Approval : " + sbuilder.ToString().Remove(sbuilder.Length - 1));
                            p_Var.sbuildertmp.Append(p_Var.sbuildertmp.ToString().Substring(0, p_Var.sbuildertmp.ToString().Length - 1));
                            miscellBL.SendEmail("", "", "no-reply.herc@nic.in;" + p_Var.sbuildertmp.ToString().Trim(), "Email For Approval", Session["Email"].ToString(), p_Var.sbuilder.ToString());
    ...
    protected void btnSendEmailsDis_Click(object sender, EventArgs e)
    {
        try
        {
                ...
                if (p_Var.Result > 0)
                {
                    ...
                    if (p_Var.sbuildertmp.ToString().EndsWith(";"))
                    {
                        p_Var.sbuilder.Append("Records are disapproved : " + sbuilder.ToString().Remove(sbuilder.Length - 1));
                        p_Var.sbuildertmp.Append(p_Var.sbuildertmp.ToString().Substring(0, p_Var.sbuildertmp.ToString().Length - 1));
